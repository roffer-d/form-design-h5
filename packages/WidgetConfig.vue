<template>
  <div class="widget-config">
    <el-form label-suffix="："
      ...
      <!--      </el-form-item>-->
      <el-form-item label="标题" v-if="data.type!='text'">
        <el-input v-model="data.label" @blur="titleBlur"
      ...
      <el-form-item label="表单栅格"
                    v-if="false && !data.subfield && !['group'].includes(data.type)">
      ...
      <el-form-item label="数据类型"
                    v-if="false && ['cascader','checkbox','upload','img','array'].includes(data.type)">
      ...
      <component :is="getComponent" :data="data" :column="column" :options="options" :key="componentKey"></component>

      <el-form-item label="是否多选" v-if="data.type=='select' && ['static','remote'].includes(data.dicOption)">
    ...
    titleBlur() {
      if (!this.data.label) {
        fields.forEach(parent => {
          parent.list.forEach(field => {
            if (field.type == this.data.type) {
              this.data.label = field.label;
            }
          })
        })
      }
