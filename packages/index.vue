<template>
  <div class="form-designer">
    <el-container style="border-top: 1px solid #ccc;">
      <!-- 左侧字段 -->
      ...
      <!-- 右侧配置 -->
      <el-aside class="widget-config-container" :width="asideRightWidth" style="color:#ccc">
        <el-tabs v-model="configTab" stretch>
          <el-tab-pane label="字段属性"
                       name="widget"
                       style="padding: 0 10px;">
            <widget-config :data="widgetFormSelect" :options="options" :column="widgetForm.column"></widget-config>
          </el-tab-pane>
          <el-tab-pane label="表单属性" v-if="false"
                       name="form"
                       lazy
                       style="padding: 0 10px;">
    ...
    <!-- 生成JSON -->
    <form-review :show-mobile-view="false" v-if="showRreviewVisible" :visible.sync="showRreviewVisible"
                 :option="widgetFormPreview" :baseURL="options.baseURL||''"></form-review>
  </div>
</template>
...
export default {
  ...
  methods: {
    filterFields() {
      if (this.fieldList.length) {
        this.fields.forEach(item => {
          item.list = item.list.filter(field => {
            return this.fieldList.includes(field.type)
          })
        })

        this.fields = this.fields.filter(item => item.list.length > 0)
      }
    },
    ...
    handlePreview() {
      if (!this.widgetForm.column || this.widgetForm.column.length == 0) this.$message.error("没有需要展示的内容")
        ...
        this.transformToAvueOptions(this.widgetForm).then(data => {
          this.widgetFormPreview = JSON.parse(JSON.stringify(data))
          this.widgetMobileFormPreview = data
          this.showRreviewVisible = true
        })

        let ueditorDom = document.querySelector('.edui-editor');
        ueditorDom && (ueditorDom.style.zIndex = 0);
      }
    },
    // 导入JSON - 弹窗 - 确定
    ...
    getFormJSON(json) {
      return this.transAvueOptionsToFormDesigner(json);
    },
    setJSON(json) {
        ...
        try {
          ...
          for (let i = 0; i < data.column.length; i++) {
            ...
            if (col.type == 'dynamic' && col.children && col.children.column && col.children.column.length > 0) {
              ...
              } else if (col.dicOption == 'form' || col.dicOption == 'data') {
                // delete col.dicData
                delete col.dicQuery
                delete col.dicUrl
                delete col.dicMethod
